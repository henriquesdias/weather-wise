...
export default function Header({ setSearch }: Headerprops) {
  ...
  return (
    <header className="text-white flex items-center justify-between p-5 pt-10 relative">
      ...
      >
        <div
          className="flex flex-col items-center bg-[#D9D9D9] max-h-48	 w-44 rounded-lg text-black overflow-y-auto"
        ...
        >
          ...
          {favoritesLocations.map((info, index) => (
            <span className="flex items-center p-2 gap-2" key={index}>
              <span
              ...
              <span
                onClick={() => {
                  ...
                  if (favoriteLocations) {
                    const favoriteLocationsParsed =
                      JSON.parse(favoriteLocations);
                    localStorage.setItem(
                      "names",
                      JSON.stringify(
                        favoriteLocationsParsed.filter(
                          (location: any) => location.id !== info.id,
                        ),
                      ),
                    );
                  }
